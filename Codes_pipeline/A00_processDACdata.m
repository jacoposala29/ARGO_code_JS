...
for iFile = 1:nFile
    ...
    profTransSys = ncread([fileList(iFile).folder,'/../',num2str(profFloatID),'_meta.nc'],'TRANS_SYSTEM');
    profTransSys = strtrim(profTransSys');
    ...
    profTransSysAggr{end+1} = profTransSys;
...
save(folder2use,'/Outputs/Argo_data_aggr.mat','startYear','endYear','nProf','profLatAggr','profLongAggr','profYearAggr','profMonthAggr','profJulDayAggr','profFloatIDAggr','profCycleNumberAggr','profModeAggr','profPresAggr','profTempAggr','profPsalAggr','profTransSysAggr','-v7.3');
